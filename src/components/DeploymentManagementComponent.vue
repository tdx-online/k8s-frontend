...
const createDeployment = () => {
  let param
  try {
    param = JSON.parse(newDeploymentData.value)
  } catch (e) {
    try {
      param = yaml.load(newDeploymentData.value)
    } catch (e) {
      ElMessage({
        message: '请输入正确的配置',
        type: 'error',
...
onMounted(() => {
  fetchDeployments()
  fetchInterval = setInterval(fetchDeployments, 5000)
})
...
<template>
  ...
  <el-dialog v-model="dialogVisible" title="新建 Deployment">
    <el-input type="textarea" autosize v-model="newDeploymentData" placeholder="请输入新建 Deployment 的配置"
