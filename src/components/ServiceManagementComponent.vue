...
const createService = () => {
  ...
  let param
  try {
    param = JSON.parse(newServiceData.value)
  } catch (e) {
    try {
      param = yaml.load(newServiceData.value)
    } catch (e) {
      ElMessage({
        message: '请输入正确的配置',
        type: 'error',
...
onMounted(()=>{
  fetchServices()
  fetchInterval = setInterval(fetchServices, 5000)
})
...
<template>
  ...
  <el-dialog v-model="dialogVisible" title="新建 Service">
    <el-input type="textarea" autosize v-model="newServiceData" placeholder="请输入新建 Service 的配置"
